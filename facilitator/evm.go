...
//
// In v2 the PaymentPayload.Payload field is a decoded map[string]interface{}
// rather than raw JSON, so we re-marshal it once here and hand the bytes to
// the scheme/evm helpers (which still operate on JSON). The re-marshal is
// cheap and keeps this commit from forcing a rewrite of scheme/evm.
...
func (t *EVMFacilitator) verifyEIP3009(ctx context.Context, payload *types.PaymentPayload, req *types.PaymentRequirements, raw []byte) (*types.PaymentVerifyResponse, error) {
	...
	// Step 2: Scheme verification (scheme lives in payload.Accepted in v2)
